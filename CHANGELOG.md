...
- **Tableaux de bord personnalisés** : un module « Tableau de bord » où chaque
  ...
  glisser-déposer + redimensionnement** (positions persistées). Création par
  **langage naturel** (l'IA génère la requête + choisit la viz) ou à la main,
  avec aperçu en direct. Persisté par (connexion + base). Widgets et
  **monitoring des tâches** regroupés sous une seule entrée.
